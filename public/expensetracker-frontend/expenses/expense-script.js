...
async function postexpense(myObj){
  try{
    ...
    console.log(token);
  let response = await axios.post('https://expense.chandraprakash.tech/user/add-expense',myObj,{ headers : {"authorization" : token } })
  console.log("posted");
  return response;
  }
  catch(err){
...
document.querySelector('#rzp-button1').onclick = async function(){
  let token = localStorage.getItem('token');
  let response = await axios.get('https://expense.chandraprakash.tech/purchase/premiummembership',{ headers : { "authorization" : token } });
  ...
  var options = {
    ...
    "handler": async function (response) {
      ...
      //  location.reload();
      await axios.post('https://expense.chandraprakash.tech/purchase/updatetransactionstatus',{
...
rzp1.on('payment.failed', async function (response){
  console.log(response.error.metadata);
        alert("somthing went wrong");
       await axios.post('https://expense.chandraprakash.tech/purchase/updatefailurestatus',{
...
document.addEventListener('DOMContentLoaded',( async function (){
  ...
  let token = localStorage.getItem('token');
  let response = await axios.get('https://expense.chandraprakash.tech/user/getdetails',{ headers : { "authorization" : token } });
