...
async function postexpense(myObj){
  try{
    ...
    console.log(token);
  let response = await axios.post('http://50.19.206.111:80/user/add-expense',myObj,{ headers : {"authorization" : token } })
  console.log("posted");
  return response;
  }
  catch(err){
...
document.querySelector('#rzp-button1').onclick = async function(){
  let token = localStorage.getItem('token');
  let response = await axios.get('http://50.19.206.111:80/purchase/premiummembership',{ headers : { "authorization" : token } });
  ...
  var options = {
    ...
    "handler": async function (response) {
      ...
      //  location.reload();
      await axios.post('http://50.19.206.111:80/purchase/updatetransactionstatus',{
...
rzp1.on('payment.failed', async function (response){
  console.log(response.error.metadata);
        alert("somthing went wrong");
       await axios.post('http://50.19.206.111:80/purchase/updatefailurestatus',{
...
document.addEventListener('DOMContentLoaded',( async function (){
  ...
  let token = localStorage.getItem('token');
  let response = await axios.get('http://50.19.206.111:80/user/getdetails',{ headers : { "authorization" : token } });
