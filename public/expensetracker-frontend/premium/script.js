...
async function showORhideLeaderBoard(){
  ...
  if(HideOrView.style.display=="none"){
    ...
    console.log(token);
    let response = await axios.get('http://50.19.206.111:80/premium/get-leaderboard',{headers : { "authorization" : token }})
...
downloadBtn.addEventListener('click',async()=>{
  try{
    let token = localStorage.getItem("token");
    let response = await axios.get('http://50.19.206.111:80/premium/download',{headers : {"authorization" : token}})
...
olderDownloads.addEventListener('click',async()=>{
  ...
  if(count>0){
    ...
    let response = await axios.get('http://50.19.206.111:80/premium/getdownloadlinks',{headers : {"authorization" : token}});
...
async function getAllExpenses(PageNo){
  try{
    if(PageNo==undefined){
      ...
      console.log(rows);
      console.log(token);
    let response = await axios.get(`http://50.19.206.111:80/user/get-expenses`,{ headers : { "authorization" : token ,"noofrows" : rows }});
    ...
    return response.data;
      ...
      console.log(token);
      console.log(rows);
    let response = await axios.get(`http://50.19.206.111:80/user/get-expenses?page=${PageNo}`,{ headers : { "authorization" : token ,"noofrows" : rows }});
    verify(response.data.headers);
    UserTotalExpensesData = response.data.expense;
    return response.data;
    }
...
async function deleteExpense(expenseid){
  ...
  let token = localStorage.getItem("token");
  console.log(token);
  let response = await axios.get(`http://50.19.206.111:80/user/delete-expense/${expenseid}`,{ headers : { "authorization" : token } });
